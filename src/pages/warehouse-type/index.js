...
import TableHeader from 'src/views/apps/warehouse-type/TableHeader'
import AddUserDrawer from 'src/views/apps/warehouse-type/AddWarehouseTypeDrawer'
import SidebarEditPeople from 'src/views/apps/warehouse-type/EditWarehouseType'
import { getAllWarehouseType } from 'src/api/RequestApi'
import { deleteWarehouseType } from 'src/api/RequestApi'
...
const PersonsType = ({ apiData }) => {
  ...
  const handleDelete = async() => {
    ...
    try {
      ...
      const response = await deleteWarehouseType(data, 1)

      if(response.status == 200){
       await getTyperPersons()
        toast.success('Tipo de alamcen eliminado con éxito')
  ...
  return (
    <Grid container spacing={6.5}>
      ...
      { openModal &&
       <DialogAlert open={openModal} title={'Desea eliminar el tipo de almacen ' + nombre} content={'Esta acción no se puede revertir'} onConfirm={handleDelete} handleClose={closeModal}/> 
        }
    </Grid>
  )
}
